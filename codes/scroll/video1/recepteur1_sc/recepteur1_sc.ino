...
#define canal 0 // Identification du même canal que pour l'émission, pour la réception
...
void setup() {
  ...
  printString(str); // Message du début, qui nous indique la fin des config  de début
  termPutchar('\r');
  delay(1000);
      ...
      case AFFICHAGE : //On passe en état d'affichage 
        ...
        //La valeur de rxlendata changera en 2 car le buffer ne contiendra que 2 octets quelque soit l'envoi
        {
          rxNbTrame++; // on incrémente le nombre de trames reçues de 1
           // Affichage du nombre de trames reçues 
          strcpy(str,"Nombre de trame :  ");
          printString(str);

          sprintf(temp, "%d", rxNbTrame);
          printString(temp);
          termPutchar('\r');

           // Affichage du nombres d'octets reçus
          strcpy(str,"nombre d'octets recu :  ");
          printString(str);

          sprintf(temp, "%u", rxlendata);
          printString(temp);
          termPutchar('\r');
          termPutchar('\r');
